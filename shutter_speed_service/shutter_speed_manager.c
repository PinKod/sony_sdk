...
int main() {
    ...
    unsigned int *shutter_speed_array = get_property_array(SHUTTER_SPEED_CODE, device_handle_handle, &shutter_speed_writable, &shutter_speed_current, &shutter_speed_array_len, &shutter_speed_get_result);
    ...
    if(get_current_value_property(SHUTTER_SPEED_CODE, device_handle_handle, &current_shutter_speed) == 0) {
    ...
    for(unsigned int i = 0; i < shutter_speed_array_len; i++) {
        ...
        set_value_property(SHUTTER_SPEED_CODE, device_handle_handle, shutter_speed_array[i], SHUTTER_SPEED_TYPE);
    }
