...
int main() {
    ...
    EXPOSURE_BIAS_COMPENSATION_TYPE* shutter_speed_array = get_property_array(EXPOSURE_BIAS_COMPENSATION_CODE, device_handle_handle, &shutter_speed_writable, &shutter_speed_current, &shutter_speed_array_len, &shutter_speed_get_result, sizeof(EXPOSURE_BIAS_COMPENSATION_TYPE));
    ...
    if (get_current_value_property(EXPOSURE_BIAS_COMPENSATION_CODE, device_handle_handle, &current_shutter_speed) == 0) {
    ...
    for (unsigned int i = 0; i < shutter_speed_array_len; i++) {
        ...
        set_value_property(EXPOSURE_BIAS_COMPENSATION_CODE, device_handle_handle, shutter_speed_array[i], EXPOSURE_BIAS_COMPENSATION_VALUE_TYPE);
    }
